...
static int stream_formatter(FILE *pipe, void *data, metric_type type, char *name, void *value) {
    ...
    switch (type) {
        case KEY_VAL:
            STREAM("stats.kv.%s|%f|%lld\n", name, *(double*)value);
            break;

        case GAUGE:
            STREAM("stats.gauges.%s|%f|%lld\n", name, ((gauge_t*)value)->value);
            break;
        ...
        case TIMER:
            t = (timer_hist*)value;
            //STREAM("stats.timers.%s.sum|%f|%lld\n", name, timer_sum(&t->tm));
            //STREAM("stats.timers.%s.sum_sq|%f|%lld\n", name, timer_squared_sum(&t->tm));
            STREAM("stats.timers.%s.mean|%f|%lld\n", name, timer_mean(&t->tm));
            //STREAM("stats.timers.%s.lower|%f|%lld\n", name, timer_min(&t->tm));
            //STREAM("stats.timers.%s.upper|%f|%lld\n", name, timer_max(&t->tm));
            STREAM("stats.timers.%s.count|%lld|%lld\n", name, timer_count(&t->tm));
            STREAM("stats.timers.%s.stdev|%f|%lld\n", name, timer_stddev(&t->tm));
            STREAM("stats.timers.%s.median|%f|%lld\n", name, timer_query(&t->tm, 0.5));
            STREAM("stats.timers.%s.p95|%f|%lld\n", name, timer_query(&t->tm, 0.95));
            //STREAM("stats.timers.%s.p99|%f|%lld\n", name, timer_query(&t->tm, 0.99));

            // Stream the histogram values
            if (t->conf) {
                STREAM("stats.timers.%s.histogram.bin_<%0.2f|%u|%lld\n", name, t->conf->min_val, t->counts[0]);
